...
function heatTextColor(value: number) { return value > 0 ? "#FFFFFF" : "#0A0D14"; }
...
export default function V3PreviewAug21() {
  ...
  return <div className="bit-v3 bit-page bg-[#f3efe6] text-[#16352f]">
    <div className="sticky top-0 z-50 border-b border-[#E4E6EA] bg-white/95 px-5 py-3 backdrop-blur sm:px-10"><div className="mx-auto flex max-w-[1200px] items-center justify-between gap-3"><button onClick={() => scrollTo("overview")} className="bit-action text-left" aria-label="BIT Market Intelligence"><img className="bit-brand-logo" src={`${import.meta.env.BASE_URL}brand/BIT-Logo-black.svg`} alt="BIT" /></button><div className="hidden font-mono text-[10px] tracking-[0.16em] text-[#59657A] sm:block">{text.edition.toUpperCase()}</div><button onClick={() => scrollTo("v3-exhibits")} className="bit-action ml-auto border border-[#0040FF] px-3 py-2 font-mono text-[10px] font-bold tracking-[.08em] text-[#0040FF]">{text.exhibit}</button></div></div>
    ...
    <main id="v3-exhibits" className="border-t-[10px] border-[#16382f] bg-[#f3efe6]">
      <section className="mx-auto max-w-[1200px] px-5 py-16 sm:px-10 lg:px-[var(--layout-padding)]"><div className="grid gap-8 lg:grid-cols-[.82fr_1.18fr]"><div><p className="section-kicker">{text.kicker}</p><h1 className="section-title mt-3 whitespace-pre-line">{text.title}</h1></div><div className="border-l-2 border-[#0040FF] pl-5"><p className="max-w-2xl text-[15px] leading-7 text-[#59657A]">{text.intro}</p><button onClick={() => scrollTo("overview")} className="bit-action mt-5 inline-flex items-center gap-2 border border-[#0040FF] px-4 py-2 text-xs font-bold text-[#0040FF]"><FileDown size={14} /> {text.full}</button></div></div></section>
      ...
      <section className="mx-auto max-w-[1200px] px-5 py-16 sm:px-10 lg:px-[var(--layout-padding)]"><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="section-kicker">{text.heatKicker}</p><h2 className="section-title mt-3 whitespace-pre-line">{text.heatTitle}</h2></div><div className="inline-flex rounded-[8px] border border-[#E4E6EA] bg-[#F5F7FA] p-1">{(["oneDay", "fiveDay", "twentyDay"] as Horizon[]).map((value) => <button key={value} onClick={() => setHorizon(value)} className={`bit-action px-4 py-2 font-mono text-[10px] font-bold ${horizon === value ? "bg-[#0040FF] text-white" : "text-[#59657A]"}`}>{value === "oneDay" ? "1D" : value === "fiveDay" ? "5D" : "20D"}</button>)}</div></div><p className="mt-5 max-w-2xl text-sm leading-7 text-[#59657A]">{text.heatBody}</p><div id="v3-heatmap-chart" className="exhibit mt-7 rounded-[12px] border border-[#E4E6EA] bg-white p-4 sm:p-6"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{orderedSectors.map((item) => <button key={item.symbol} onMouseEnter={() => setSector(item)} onFocus={() => setSector(item)} onClick={() => setSector(item)} className="min-h-24 rounded-[8px] p-4 text-left transition hover:scale-[1.01]" style={{ background: heatColor(item[horizon]), color: heatTextColor(item[horizon]) }}><p className="font-mono text-[10px] font-bold tracking-[.12em]">{item.symbol}</p><p className="mt-2 text-sm font-bold">{item.label[language]}</p><p className="mt-3 font-mono text-lg font-semibold">{pct(item[horizon])}</p></button>)}</div></div><div className="mt-5 grid gap-4 border-l-2 border-[#0040FF] pl-5 sm:grid-cols-[180px_1fr]"><p className="font-mono text-[10px] tracking-[.14em] text-[#0040FF]">{text.selected.toUpperCase()}</p><div><p className="font-display text-2xl">{sector.symbol} / {sector.label[language]}</p><p className="mt-2 font-mono text-sm text-[#0040FF]">1D {pct(sector.oneDay)} · 5D {pct(sector.fiveDay)} · 20D {pct(sector.twentyDay)}</p></div></div></section>
