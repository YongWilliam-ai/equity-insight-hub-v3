/**
 * EQUITY / INSIGHT — independent V3 research edition, 21 Aug 2026.
...
function heatColor(value: number) {
  if (value >= 1.5) return "#2f735c";
  if (value > 0) return "#9bbb94";
  if (value > -0.75) return "#ded3c2";
...
function exportSvg(elementId: string, filename: string) {
  ...
  image.onload = () => { const canvas = document.createElement("canvas"); canvas.width = width * 2; canvas.height = height * 2; const context = canvas.getContext("2d"); if (!context) return; context.scale(2, 2); context.fillStyle = "#f7f1e6"; context.fillRect(0, 0, width, height); context.drawImage(image, 0, 0, width, height); const link = document.createElement("a"); link.href = canvas.toDataURL("image/png"); link.download = filename; link.click(); URL.revokeObjectURL(url); };
  image.src = url;
}
...
export default function V3PreviewAug21() {
  ...
  return <div className="bg-[#f3efe6] text-[#16352f]">
    <div className="sticky top-0 z-50 border-b border-[#ded4c2] bg-[#f7f1e6]/95 px-5 py-3 backdrop-blur sm:px-10"><div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3"><button onClick={() => scrollTo("overview")} className="font-display text-sm text-[#16352f]">EQUITY / INSIGHT</button><div className="hidden font-mono text-[10px] tracking-[0.16em] text-[#6c786f] sm:block">{text.edition.toUpperCase()}</div><button onClick={() => scrollTo("v3-exhibits")} className="ml-auto border border-[#c99a48] px-3 py-2 font-mono text-[10px] font-bold tracking-[.08em] text-[#775d2b]">{text.exhibit}</button></div></div>
    ...
    <main id="v3-exhibits" className="border-t-[10px] border-[#16382f] bg-[#f3efe6]">
      ...
      <section className="border-y border-[#e1d7c5] bg-[#eae4d8] px-5 py-16 sm:px-10 lg:px-14"><div className="mx-auto grid max-w-[1500px] gap-8 xl:grid-cols-[1.1fr_.9fr]"><div><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="section-kicker">{text.breadthKicker}</p><h2 className="section-title mt-3 whitespace-pre-line">{text.breadthTitle}</h2></div><button onClick={() => exportSvg("v3-breadth-chart", "equity-insight-v3-breadth-2026-08-20.png")} className="inline-flex items-center gap-2 border border-[#b79045] px-3 py-2 font-mono text-[10px] font-bold tracking-[.1em] text-[#775d2b]"><Download size={14} /> {text.export}</button></div><p className="mt-5 max-w-2xl text-sm leading-7 text-[#68756f]">{text.breadthBody}</p><div id="v3-breadth-chart" className="exhibit mt-6 bg-[#f9f4ea] p-4 sm:p-6"><ResponsiveContainer width="100%" height={310}><LineChart data={breadthTrend} margin={{ top: 12, right: 12, bottom: 6, left: -14 }}><CartesianGrid stroke="#ded3c2" strokeDasharray="2 5" /><XAxis dataKey="date" tick={{ fill: "#68756f", fontFamily: "monospace", fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis yAxisId="breadth" domain={[0, 100]} tickFormatter={(value) => `${value}%`} tick={{ fill: "#68756f", fontFamily: "monospace", fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis yAxisId="spy" orientation="right" tickFormatter={(value) => `${value}%`} tick={{ fill: "#8b7a64", fontFamily: "monospace", fontSize: 10 }} axisLine={false} tickLine={false} /><Tooltip contentStyle={{ background: "#10342d", border: "1px solid #c99a48", borderRadius: 0, color: "#f7f0e3" }} labelStyle={{ color: "#e0c27b" }} /><Line yAxisId="breadth" dataKey="five" name={text.five} stroke="#b58a42" strokeWidth={2.4} dot={false} activeDot={{ r: 5 }} /><Line yAxisId="breadth" dataKey="twenty" name={text.twenty} stroke="#2f735c" strokeWidth={2.4} connectNulls dot={false} activeDot={{ r: 5 }} /><Line yAxisId="spy" dataKey="spy" name={text.spy} stroke="#b85b4e" strokeWidth={1.5} strokeDasharray="4 4" dot={false} /></LineChart></ResponsiveContainer></div><p className="mt-3 font-mono text-[10px] tracking-[.08em] text-[#7a827e]">{text.latest}</p></div><aside className="bg-[#153a32] p-7 text-[#f7f0e3] sm:p-9"><p className="font-mono text-[10px] tracking-[.18em] text-[#e0c27b]">{text.source.toUpperCase()}</p><p className="mt-5 text-sm leading-7 text-[#d4ded7]">{text.sourceBody}</p><p className="mt-7 border-t border-white/15 pt-4 text-xs leading-5 text-[#c2d0c8]">{text.disclaimer}</p></aside></div></section>
